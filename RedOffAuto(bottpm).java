//hippity bobbity coming for that swaggity: I already reversed the necessary values, the only thing is values 
//for the turn to cryptobox
...
@Autonomous(name = "RedOffAuto")

public class RedOffAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if(relicMove ==  2){ //NOW IS LEFT, FARTHEST COLUMN, NOT RIGHT
            spin(0.5);
            Thread.sleep(300); //needs to be a really small-big value
            stopRobo();

        } else if(relicMove == 1){
            spin(0.5);
            Thread.sleep(200); //needs to be a slightly less value
            stopRobo();


        } else if(relicMove == 0){ //NOW IS RIGHT, CLOSEST COLUMN, NOT LEFT
            spin(0.5);
            Thread.sleep(100); //lol YEET YAH
            stopRobo();
        }
        ...
        //code to run after robot in position
